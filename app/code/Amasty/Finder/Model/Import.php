...
class Import
{
    ...
    protected function _parseFile($fp, &$countProcessedRows, &$countProcessedRowsInCurrentFile, &$countProcessedLinesInCurrentFile, $countDropDowns, $fileLog, $ranges)
    {
        ...
        while (($line = fgetcsv($fp, \Amasty\Finder\Model\Resource\Finder::MAX_LINE, ',', '"')) !== false && $countProcessedRows < $this->scopeConfig->getValue('amfinder/import/max_rows_per_import')) {
    ...
    protected function _insertValues($names, $dropdowns)
    {
         ...
         */
        $db = $this->objectManager->create('Amasty\Finder\Model\Resource\Finder')->getConnection();
        ...
        for ($j=0; $j < $countDropDowns; ++$j){ // columns
            ...
            for ($i=0; $i < $namesIndex; ++$i){ //rows
                $key = $parents[$i][$j] . '-' . mb_strtolower($names[$i][$j]);
    ...
    public function runAll()
    {
        ...
        /**
         * @var $collectionFinder \Amasty\Finder\Model\Resource\Finder\Collection
         */
        ...
        /**
         * @var $collection \Amasty\Finder\Model\Resource\ImportLog\Collection
         */
    ...
    public function clearOldData(\Amasty\Finder\Model\Finder $finder)
    {
        ...
        $this->objectManager->create('Amasty\Finder\Model\Resource\Value')->deleteValuesByDropDowns($ids);
    }
    ...
    {
        return $this->objectManager->create('Amasty\Finder\Model\Resource\ImportLog')->getTable($tableName);
    }
