...
class Config implements ObserverInterface
{
	...
	public function execute(EventObserver $observer)
	{
		...
		if ($args && $this->request->getParam('groups')['api']['fields'])
		{
			$errors = [];
			...
			if ('' === (string) $this->_helper->getConfig('ec/api/google_gtm_account_id'))
			{
				$errors[] = __('Google Tag Manager Account ID is not set.');
			}
			
			if ('' === (string) $this->_helper->getConfig('ec/api/google_gtm_ua'))
			{
				$errors[] = __('Google Tag Manager Universal Analytics Tracking ID is not set.');
			}
			
			if ('' === (string) $this->_helper->getConfig('ec/api/google_gtm_container'))
			{
				$errors[] = __('Google Tag Manager Container ID is not set.');
			}
			else 
			{
				if (!is_numeric($this->_helper->getConfig('ec/api/google_gtm_container')))
				{
		...
		if (!$this->moduleManager->isOutputEnabled('Anowave_Ec'))
		{
			$this->_messageManager->addWarningMessage('The output of Anowave_Ec module is disabled. Tracking will not work.');
		}
