...
class CsvImportHandler
{
    ...
    public function getRowCategoryimport($option, $recordID) {
        //global $rec_id;
        ...
        switch ($option) {
            case "1":  // Get Record to process
                $query = "SELECT `rootid`, `store`, `name`, `categories`, `level`, `position`, `description`, `url_key`, `is_active`, `display_mode`, `page_layout`, `cms_block`, `is_anchor`, `meta_title`, `meta_keywords`, `meta_description`, `category_image`, `include_in_menu`, `custom_layout_update`, `custom_design`, `custom_use_parent_settings`, `custom_apply_to_products`, `category_products`, `rec_id` FROM catalog_import WHERE is_processed=0 LIMIT 1";
                break;
            case "2":
                // Not used currently!
            ...
            case "4":
                ...
                break;
            case "5":
                $query = "SELECT COUNT(*) c FROM catalog_category_entity_varchar WHERE attribute_id = 45 AND `value` = '" . $recordID ."'";
                // update to 2 to indicate it has been successfully processed
                break;
            default:
        ...
        $tblcatimport = $_connection->getTableName('catalog_import_bad');
    ...
    public function importFromImportTable($params) {
        ...
        if ($ttlnumrectogo === 0){
            ...
            setcookie($cookiename,'',time() - 4000);
...
//   leave this for the end in case we have any issues
        ...
        for ($recnum = 1; $recnum <= $numrecordstoprocess; $recnum++) {
            ...
            foreach ($ratesData as $dataRow) {
                ...
                if (empty($dataRow['name'])) {
                    //example: Boat Covers/Aquaforce/Bay Runner 18/2002
                    $output = array_pop(explode("/",$dataRow['categories']));
                    #echo $output;
                    $dataRow['name'] = $output;
                    #echo($dataRow['name']);
                    //die("test");
                }
                if (empty($dataRow['url_key'])) {
                    $dataRow['url_key'] = $dataRow['name'];
                }
                
                // Look for duplicates so we can fix url_key to make it unique!
                // Fix for error: "URL key for specified store already exists."
                $option = "5";
                $is_duplicate_key = $this->getRowCategoryimport($option, $dataRow['url_key']);
                #echo($is_duplicate_key[0]['c']);
                #print_r($is_duplicate_key);
                #die("test dupkey");
                
                if ($is_duplicate_key[0]['c'] > 1){
                    // Found more than 1 duplicate category name; add unique character(s) + dash(-) to url_key
                    //  in order to prevent an error on insert into the url_rewrite table.
                    $numcharstoadd = 1;  // Change to determined number of letters to add to url_key
                    $uselettersonly = false; // Set to true to use only Letters
                                       
                    $dataRow['url_key'] = $dataRow['name'];
                    #echo("urlkey=" . $dataRow['url_key'] . "<br>");
                    if ($uselettersonly === 0) {
                        $characters = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
                        $randomchramt = 61;
                    } else {  // $numcharstoadd = 1
                        $characters = 'abcdefghijklmnopqrstuvwxyz';
                        $randomchramt = 26;
                    }
                    $uniqueResult = '';
                    for ($i = 0; $i < $numcharstoadd; $i++) {
                        $uniqueResult .= $characters[mt_rand(0, $randomchramt)];
                    }
                    $dataRow['url_key'] = $uniqueResult . "-" . $dataRow['name'];
                    #echo($dataRow['url_key'] . "<br>");
                    #die();
                }
                #die("die anyway");
                #print_r($dataRow);
        ...
        if (($ttlnumrectogo - $recnum) <= 0) {
            // delete cookie
            #echo("deleted cookie<br>");
            setcookie($cookiename, '', time() - 3600);
            #echo("No More Records to Process!");
            setcookie($cookiename2, "0 (Finished!)", time() + 3600, "/");
        } else {
            setcookie($cookiename, $max_categories, time() + 3600, "/");
            setcookie($cookiename2, $ttlnumrectogo - $recnum, time() + 3600, "/");
        }
    }
    ...
    protected function _importRate(array $Data, array $params)
    {
        ...
        if (empty($this->_categoryCache[$store->getId()])) {
            // I changed line 474 below back to the original code; using Like% instead of "="
		...
		*/
        ...
        if ($categoryModel->getId()) {
			...
			if(isset($Data['category_id']) && $Data['category_id'] !="") {
				...
				if($oldchilderncount < 0) { $oldchilderncount = 0; }
				$categoryNew->delete();
				...
				if(isset($Data['url_key'])) { $generalupdateidset['url_key'] = $Data['url_key'];  }
				if(isset($Data['is_anchor'])) { $generalupdateidset['is_anchor'] = $Data['is_anchor']; }
				if(isset($Data['cms_block'])) { $generalupdateidset['landing_page'] = $Data['cms_block']; }
				if(isset($Data['display_mode'])) { $generalupdateidset['display_mode'] = $Data['display_mode']; }
				if(isset($Data['meta_title'])) { $generalupdateidset['meta_title'] = $Data['meta_title']; }
				if(isset($Data['meta_keywords'])) { $generalupdateidset['meta_keywords'] = $Data['meta_keywords']; }
				if(isset($Data['meta_description'])) { $generalupdateidset['meta_description'] = $Data['meta_description']; } 
				if(isset($Data['page_layout'])) { $generalupdateidset['page_layout'] = $Data['page_layout']; }
				if(isset($Data['custom_layout_update'])) { $generalupdateidset['custom_layout_update'] = $Data['custom_layout_update']; }
				if(isset($Data['custom_design'])) {	$generalupdateidset['custom_design'] = $Data['custom_design']; }
				if(isset($Data['include_in_menu'])) { $generalupdateidset['include_in_menu'] = $Data['include_in_menu']; }
				if(isset($Data['custom_apply_to_products'])) { $generalupdateidset['custom_apply_to_products'] = $Data['custom_apply_to_products']; }
				if(isset($Data['custom_use_parent_settings'])) { $generalupdateidset['custom_use_parent_settings'] = $Data['custom_use_parent_settings']; }
				if(isset($Data['position'])) { 
					if($Data['position'] != "") { 
			...
			foreach ($categories as $catName) {
				...
				if (empty($cache[$namePath])) {
					...
					if(isset($Data['page_layout'])) { $categoryNew->setData('page_layout', $Data['page_layout']); }
					if(isset($Data['cms_block'])) { $categoryNew->setData('landing_page', $Data['cms_block']); }
					if(isset($Data['image'])) { $categoryNew->setData('image', $Data['image']); }
					if(isset($Data['meta_title'])) { $categoryNew->setData('meta_title', $Data['meta_title']); }
					if(isset($Data['meta_keywords'])) { $categoryNew->setData('meta_keywords', $Data['meta_keywords']); }
					if(isset($Data['meta_description'])) { $categoryNew->setData('meta_description', $Data['meta_description']); }
					if(isset($Data['include_in_menu'])) { $categoryNew->setData('include_in_menu', $Data['include_in_menu']); }
		...
		if ($categoryModel->getId() > 0) {
			$categoryModel->addData($modelData);
			if(isset($Data['url_key'])) { $categoryModel->setUrlKey($Data['url_key']); }
