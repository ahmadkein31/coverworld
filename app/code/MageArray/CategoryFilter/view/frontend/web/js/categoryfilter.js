    ...
    jQuery.widget('magearray.categoryfilter', {
        ...
        _loadNextDropDown: function (event, selectedValue, labelembedded, nameInLayout, redirectUrl, dataId, levels) {
            ...
            var selectedValue = selectedValue;
             if (selectedValue != "") {
                 var previousLabelText = jQuery('#' + nameInLayout + dataId + " option:selected").text();
               	 }
            ...
            } else if (previousLabelText.indexOf("All Years") >= 0 || previousLabelText.indexOf("THRU-") >= 0) { 
                    //console.log(previousLabelText);
                    window.location.href = redirectUrl;
                    ...
                    jQuery.ajax({
                        ...
                        success: function (data) {
                            if (data.length) {
                                ...
                                jQuery('#' + nameInLayout + dataId).removeClass('dropdown-selected');
                                jQuery('#' + nameInLayout + dataId).addClass('dropdown');
                                jQuery('#' + nameInLayout + dataId).blur();
                                jQuery('#' + nameInLayout + nextDropdown).removeClass('dropdown');
