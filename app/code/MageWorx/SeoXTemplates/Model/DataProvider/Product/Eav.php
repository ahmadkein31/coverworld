...
class Eav extends \MageWorx\SeoXTemplates\Model\DataProvider\Product
{
    ...
    public function getData($collection, $template, $customStoreId = null)
    {
        ...
        $productIds       = array_keys($this->_collection->toOptionHash());
        ...
        foreach ($attributes as $attribute) {
            ...
            $select = $connection->select(['entity_id'])
                ...
                ->where("attribute_id = '$attribute[attribute_id]' AND entity_id IN ({$productIdsString}) AND store_id = {$this->_storeId} AND CHAR_LENGTH(value) > 0");
