...
class Product extends \MageWorx\SeoXTemplates\Model\AbstractTemplate
{
    ...
    /**
     * Retrive array template type ID => template title
     *
     * @return array
     */
    ...
    /**
     * Retrive type title such as "Product URL Key", "Product Meta Title", ... by type ID
     *
    ...
    /**
     * Retrive attribute list for template type ID
     *
     * @param int|null $typeId
     * @return array|null
     */
    ...
    /**
     * Retrive default values for create
     *
     * @return array
     */
    ...
    public function afterSave()
    {
        if ($this->isAssignForAllItems() && $this->getDuplicateTemplateAssignedForAll()) {
            $this->messageManager->addError(__('The template cannot be saved. There is another template assigned for all products.'));
        }
        ...
        elseif ($this->getId() && $this->isAssignForIndividualItems()) {
            ...
            if ($oldAssignType == $newAssignType) {
                ...
                if (array_intersect(array_map('intval', $itemIds), array_map('intval', $analogItemIds))) {
                    $this->messageManager->addError(__('The template was saved without assigned products. Please add products manually.'));
                    return parent::afterSave();
                }
            }
    ...
    protected function _getExcludeItemIdsByTemplate($nestedStoreId = null)
    {
        ...
        if ($this->getStoreId() == '0') {
            ...
            if ($this->isAssignForIndividualItems()) {
                ...
                $templateCollection->addAssignTypeFilter(array(self::ASSIGN_INDIVIDUAL_ITEMS, self::ASSIGN_GROUP_ITEMS));
            }
            elseif ($this->isAssignForGroupItems()) {
