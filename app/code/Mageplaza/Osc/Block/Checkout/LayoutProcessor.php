...
class LayoutProcessor implements \Magento\Checkout\Block\Checkout\LayoutProcessorInterface
{
	...
	private function addAddressOption(&$fields)
	{
		...
		if(isset($fields['region_id']['label'])){
			$fields['region_id']['label']= __('State');
		}
		if(isset($fields['street']['label'])){
			$fields['street']['label']= __('Address');
		}
		if(isset($fields['postcode']['label'])){
			$fields['postcode']['label']= __('Zip Code');
		}
		if(isset($fields['telephone']['label'])){
			$fields['telephone']['label']= __('Phone');
		}
		$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
		$region = $objectManager->create('Magento\Directory\Model\Region');
		$excludeRegions = array('AS','AF','AA','AC','AE','AM','AP','DC','FM','GU','MH','MP','PW','VI');

		foreach($excludeRegions as $key => $value){
			 $regionId = $region->loadByCode($value,'US')->getId();
			 $excludeRegionsId[]= $regionId;
		}
		$excludeRegionsId = array_merge(array_unique($excludeRegionsId),[8,9,10]);

		foreach($fields['region_id']['options'] as $key => $regionData){
			if(isset($regionData['country_id'])){
				if(($regionData['country_id'] == 'US' && in_array($regionData['value'],$excludeRegionsId))){
					$dataTmp[] = $regionData;
					unset($fields['region_id']['options'][$key]);
				}
