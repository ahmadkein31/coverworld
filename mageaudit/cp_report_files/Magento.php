...
class Magento extends Audit {
    ...
    function __construct() {
        ...
        $ch = curl_init('https://www.customerparadigm.com/code-audit/community.txt');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $community = curl_exec($ch);
        curl_close($ch);


        $ch = curl_init('https://www.customerparadigm.com/code-audit/enterprise.txt');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    protected function loadCleanMagento() {
        if (!$this->_magentoLoaded) {
            ...
            $rzip = "http://code-audit.customerparadigm.com/code-audit/versions/" . $this->_version . ".zip";
    ...
    protected function compareFiles($sourceDir) {
        ...
        try {
            $directories = new RecursiveIteratorIterator(
                    new ParentIterator(new RecursiveDirectoryIterator($sourceDir)), RecursiveIteratorIterator::SELF_FIRST);
...
#		$total = Mage::getModel('catalog/product')->getCollection()->count();
    ...
    protected function getSalesData() {
        ...
        $firstsale = $this->db()->fetchAll("SELECT * FROM `".$this->_resource->getTableName('sales_flat_order_item')."` LIMIT 1");

        $stores = $this->db()->fetchAll("SELECT * FROM `".$this->_resource->getTableName('core_store')."` ");
